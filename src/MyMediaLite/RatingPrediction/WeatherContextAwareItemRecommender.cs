		...
		public void createTimeFeatureMapper (bool time = false)
		{
			...
			if (!time) {
				...
				foreach (string timestamp in timestamps) {
					var time2 = new DateTime (int.Parse (timestamp) * 10000000L).AddYears (1969);
					var offset = TimeZone.CurrentTimeZone.GetUtcOffset (time2);
					if (!timeFeatureClassMapper.ContainsKey (time2 - offset))
						timeFeatureClassMapper.Add (time2 - offset, mapFeatureInRange (double.Parse (feature [i])));
					i++;
				}
			} else {
				res = conn.Select ("select hw.unix_utc_timestamp, " + weatherFeature + " from DAILY_WEATHER" +
